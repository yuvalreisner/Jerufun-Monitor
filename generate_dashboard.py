...
from config import SHABBAT_STATIONS


def build_data() -> dict:
    latest       = db.get_latest_snapshot()
    daily_net    = db.get_daily_network_summary()
    daily_rides  = db.get_daily_rides()
    ...
    return {
        ...
        "daily_rides": daily_rides.to_dict(orient="records") if not daily_rides.empty else [],
...
<!-- ══ 4. RIDES ══════════════════════════════════════════════════════════════ -->
<section class="sec" id="rides">
  <h2 class="sec-title">🚴 כמות נסיעות משוערכת לאורך זמן</h2>
  <div class="card">
    <div class="info-btn">i<span class="tip">כל ירידה בכמות האופניים בתחנה בין שתי מדידות עוקבות נספרת כנסיעה. אם כמות האופניים בתחנה מסוימת בין שתי מדידות לא השתנתה, לא ייספרו נסיעות — יכול להתקיים במצב בו אותה כמות אופניים נלקחה והוחזרה באותה תחנה.</span></div>
    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:10px">
      <div class="card-title" style="margin-bottom:0"></div>
      <div class="toggle-group" id="gran-rides">
        <button data-g="hourly">שעתי</button>
        <button class="active" data-g="daily">יומי</button>
        <button data-g="weekly">שבועי</button>
        <button data-g="monthly">חודשי</button>
    ...
    <div class="chart-wrap"><canvas id="chart-rides" height="280"></canvas></div>
  </div>
</section>
...
<section class="sec" id="ranking">
  ...
  <div style="display:flex;gap:10px;align-items:center;flex-wrap:wrap;margin-bottom:14px">
    ...
    <button id="rank-sort-btn" class="active" style="padding:6px 14px;border-radius:20px;border:1.5px solid #ddd;background:#222;color:#fff;font-size:13px;cursor:pointer;font-family:inherit">↓ תחנות מלאות</button>
  </div>
...
<section class="sec" id="chronic-empty">
  ...
  <div style="display:flex;gap:10px;align-items:center;flex-wrap:wrap;margin-bottom:14px">
    <div class="toggle-group" id="chronic-filter" style="margin-bottom:0">
      <button data-f="all">הכל</button>
      <button class="active" data-f="electric">⚡ חשמלי</button>
      <button data-f="regular">🚲 רגיל</button>
    </div>
    <label style="font-size:13px;color:var(--ink);display:flex;align-items:center;gap:6px">
      מעל
      <input id="chronic-days" type="number" value="1" min="1" max="365"
  ...
  <div class="card" style="padding:16px 20px">
    <div class="info-btn">i<span class="tip">מציג תחנות שבX ימים רצופים האחרונים הממוצע היומי שלהן היה פחות מ-0.5 אופניים (לפי הפילטר הנבחר). כלומר, גם תחנה שהיה בה אופניים בודד לשעה קצרה ביום נחשבת "ריקה" לצורך חישוב זה. אם יום אחד הממוצע עלה על 0.5 — הסטריק נשבר והיא לא תופיע.</span></div>
...
document.getElementById('rank-sort-btn').addEventListener('click', function() {
  if (rankSortDir === 'desc') {
    rankSortDir = 'asc';
    this.textContent = '↑ תחנות ריקות';
  } else {
    rankSortDir = 'desc';
    this.textContent = '↓ תחנות מלאות';
  }
    ...
    plugins:{
      ...
      datalabels:{
        display: function(ctx) {
          ...
          const lastIdx = data.reduce((a,_,i) => data[i] != null ? i : a, -1);
          if (ctx.dataIndex === lastIdx) return true;
          if (v === max && data.indexOf(max) === ctx.dataIndex) return true;
          if (v === min && data.indexOf(min) === ctx.dataIndex) return true;
      ...
      x:{type:'category',grid:{color:'#eeefe9'},
         ticks:{maxTicksLimit:18,color:MUTED,font:{size:10},
           callback: function(val){
             ...
             if(!lbl) return '';
             const p=lbl.split('-');
             if(p.length!==3) return lbl;
             const formatted=parseInt(p[2])+'/'+parseInt(p[1]);
             return new Date(lbl).getDay()===0 ? [formatted,'ראשון'] : formatted;
           }}}
    },
...
// ── RIDES PER HOUR/DAY/WEEK/MONTH ────────────────────────────────────────────
let ridesChart = null;
function renderRides(gran) {
  const cutoff = gran === 'hourly'
    ? new Date(Date.now() - 48 * 3600 * 1000).toISOString().slice(0,16)
    : gran === 'daily'
    ? new Date(Date.now() - 14 * 86400 * 1000).toISOString().slice(0,10)
    : null;
  const allHours = DATA.daily_rides;
  const filtered = cutoff ? allHours.filter(r => r.hour.slice(0, cutoff.length) >= cutoff) : allHours;
  const grouped = {};
  filtered.forEach(r => {
    let key;
    ...
    if (gran === 'hourly') {
      ...
      key = r.hour.slice(0,10);
    } else if (gran === 'weekly') {
      const dow = d.getDay() === 0 ? 6 : d.getDay() - 1;
      const mon = new Date(d); mon.setDate(d.getDate() - dow);
      key = mon.toISOString().slice(0,10);
    } else {
      key = r.hour.slice(0,7);
    }
    if (!grouped[key]) grouped[key] = {rides:0, elec:0, reg:0};
    grouped[key].rides += r.rides;
    grouped[key].elec  += r.elec;
    grouped[key].reg   += r.reg;
  });
  const rows = Object.entries(grouped).sort(([a],[b])=>a<b?-1:1).map(([k,v])=>({date:k,...v}));
  const labels = rows.map(r => r.date);
  ...
  base.plugins.datalabels = {
    display: (ctx) => {
      for (let i = 1; i >= 0; i--) {
        if (!ctx.chart.getDatasetMeta(i).hidden) return ctx.datasetIndex === i;
      }
      return false;
    },
    anchor: 'end',
    ...
    offset: 2,
    textAlign: 'center',
    font: { size: 11, family: "'IBM Plex Sans',sans-serif", weight: 'bold' },
    color: 'var(--ink)',
    formatter: (v, ctx) => {
      let total = 0;
      ctx.chart.data.datasets.forEach((ds, i) => {
        if (!ctx.chart.getDatasetMeta(i).hidden) total += ds.data[ctx.dataIndex] || 0;
      });
  ...
  // build x scale: stacked + granularity-specific tick formatting
  const xScale = { ...(base.scales.x || {}), stacked: true };
  if (gran === 'hourly') {
    xScale.ticks = {
      ...(xScale.ticks || {}),
      maxTicksLimit: 48,
      ...
      callback: function(val) {
        ...
        if (!lbl) return '';
        const [datePart, timePart] = lbl.split('T');
        const [, m, d] = datePart.split('-');
        return `${parseInt(d)}/${parseInt(m)} ${timePart}`;
      }
    };
  }
  base.scales.x = xScale;
  base.scales.y = { ...(base.scales.y || {}), stacked: true };
  ridesChart = new Chart(document.getElementById('chart-rides'), {
    type:'bar',
    data:{
      labels,
      datasets:[
        {label:'חשמליות', data:rows.map(r=>r.elec),
         backgroundColor:'rgba(240,180,41,.85)', borderRadius:0, stack:'rides'},
        {label:'רגילות', data:rows.map(r=>r.reg),
         backgroundColor:'rgba(41,128,185,.75)', borderRadius:3, stack:'rides'},
      ]
    },
    options: base
  });
...
renderRides('daily'); // default granularity for rides chart
...
setupGranToggle('gran-rides', renderRides);
